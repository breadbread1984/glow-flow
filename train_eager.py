...
def main():
    ...
    glow = tfp.bijectors.Invert(Glow(input_shape = (32,32,1), levels = 2));
    ...
    while True:
        for (images,_) in trainset:
            with tf.GradientTape() as tape:
                ...
                loss = -tf.math.reduce_mean(base_dist.log_prob(code));
            avg_loss.update_state(loss);
            #write log
            if tf.equal(optimizer.iterations % 10, 0):
                with log.as_default():
                    tf.summary.scalar('loss',avg_loss.result(), step = optimizer.iterations);
                    samples = glow.forward(base_dist.sample((1,8,8,8)));
                    tf.summary.image('sample', samples, step = optimizer.iterations);
