...
class GlowStep(tfp.bijectors.Bijector):
	def __init__(self, shape, depth = 2, validate_args = False, name = 'GlowStep'):
		...
		for i in range(depth):
			...
			layers.append(ConvolutionInvertible(shape[-1]));
			layers.append(tfb.Reshape(event_shape_out = [-1,np.prod(shape[1:])]));
			layers.append(tfb.RealNVP(num_masked = np.prod(shape[1:]) // 2, shift_and_log_scale_fn = tfp.bijectors.real_nvp_default_template(hidden_layers = [512,512])));
			layers.append(tfb.Reshape(event_shape_out = shape));
